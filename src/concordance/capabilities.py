...
def statement(surface: str = "secular") -> Dict[str, Any]:
    ...
    return {
        ...
        "boundaries": {
            ...
            "no_model_in_the_loop": True,
            "structuring_is_deterministic": ("a plain-language claim is reduced to checkable form by "
                      "deterministic extractors — POST /verify with a free-text `claim`, or /audit for "
                      "a whole document — with no model and no API key. An unparseable claim is "
                      "declared or FOUND as sourced cards, never guessed by a model; structured "
                      "{steps}/{mode,params} skip structuring entirely. So an agent never needs an "
                      "LLM to use this engine — enforced by the no-model-in-the-loop guard test."),
