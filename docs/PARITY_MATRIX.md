...
| **Keeping / library** | `cards_browse`, `card_get`, `card_notes`, `card_connections`, `cards_stats`, `cards_walk`, `search`, `locate`, `library_health`, `daily_card` | **PARTIAL** — 2.0 has `search` over the full 11,138-card corpus. Missing: card_get/browse/notes/connections/walk, daily_card, locate as tools | **Yes** — the keep/library is what users depend on |
