...
| **Keeping / library** | `cards_browse`, `card_get`, `card_notes`, `card_connections`, `cards_stats`, `cards_walk`, `search`, `locate`, `library_health`, `daily_card` | **MOSTLY PORTED** (2026-06-28, commit 099aa91) — 2.0 now has `search`, `card_get`, `cards_browse`, `cards_stats`, `daily_card` over the 11,138-card corpus (HTTP + MCP). Plus a browser **library page** (site/library.html) and card_connections / locate / library_health (HTTP + MCP). Still missing: card_notes (needs a notes store), cards_walk (guided walk). | mostly resolved |
