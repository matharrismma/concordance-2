...
| **Keeping / library** | `cards_browse`, `card_get`, `card_notes`, `card_connections`, `cards_stats`, `cards_walk`, `search`, `locate`, `library_health`, `daily_card` | **MOSTLY PORTED** (2026-06-28, commit 099aa91) — 2.0 now has `search`, `card_get`, `cards_browse`, `cards_stats`, `daily_card` over the 11,138-card corpus (HTTP + MCP). Still missing: card_notes, card_connections, cards_walk, locate, library_health; and a browser **library UI page**. | partially (a browse UI still blocks) |
| **Atlas / map** | `atlas`, `atlas_path`, `atlas_paths` | **NONE** (grid.py exists internally for axis-coords only) | If the map is a user-facing surface — yes |
