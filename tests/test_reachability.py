...
UNLISTED_PAGES = {
    ...
    "provision.html",     # Provision — the work/livelihood hall (ship-rate, the stall, the Masters flywheel), the same engine as golf.html applied to a man's work; a self-contained offline tool reached by direct URL / home-screen install and from the Halls, not the public nav
    "wisdom.html",        # Wisdom — the Scripture/crown hall (Treasure/Understand/Live); treasure + live are offline localStorage, Understand fetches the scripture-depth API (/passage, /cross_refs, /original, /commentary, /word_study); reached by direct URL / home-screen install and from the Halls, not the public nav
